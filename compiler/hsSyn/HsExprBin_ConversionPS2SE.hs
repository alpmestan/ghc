...
-- Conversion from GhcPs ASTs to GhcSe ASTs
...
cvRoleAnnotDecl
  :: RoleAnnotDecl GhcPs -> Conv (RoleAnnotDecl GhcSe)
...
cvFamInstEqn
  :: ( XCFamEqn GhcPs (HsTyPats GhcPs) a
     ...
     , XHsIB GhcPs (FamEqn GhcPs (HsTyPats GhcPs) a)
...
cvHsArg _ g (HsTypeArg a b) = HsTypeArg a <$> g b
...
cvLHsExpr
  :: LHsExpr GhcPs -> Conv (LHsExpr GhcSe)
cvLHsExpr = traverse cvHsExpr

cvHsExpr
  :: HsExpr GhcPs -> Conv (HsExpr GhcSe)
...
cvLit (HsInteger a b c) = HsInteger a b <$> typeToIfaceType c
cvLit (HsRat a b c) = HsRat a b <$> typeToIfaceType c
...
cvMatchGroup
  :: ( XMG GhcPs a ~ XMG GhcSe b
     , XCMatch GhcPs a ~ XCMatch GhcSe b, XCGRHSs GhcPs a ~ XCGRHSs GhcSe b
     , XCGRHS GhcPs a ~ XCGRHS GhcSe b, XXMatchGroup GhcPs a ~ XXMatchGroup GhcSe b
     , XXMatch GhcPs a ~ XXMatch GhcSe b, XXGRHSs GhcPs a ~ XXGRHSs GhcSe b
...
cvMatch
  :: ( XCMatch GhcPs a ~ XCMatch GhcSe b
     , XCGRHSs GhcPs a ~ XCGRHSs GhcSe b, XCGRHS GhcPs a ~ XCGRHS GhcSe b
     , XXMatch GhcPs a ~ XXMatch GhcSe b, XXGRHSs GhcPs a ~ XXGRHSs GhcSe b
     , XXGRHS GhcPs a ~ XXGRHS GhcSe b
     )
...
cvHsValBindsLR e@(XValBindsLR _) =
  unsupported "XValBindsLR" "HsValBindsLR" (ppr e)
...
cvIPBind
  :: IPBind GhcPs -> Conv (IPBind GhcSe)
...
cvHsBindLR e@(AbsBinds {}) =
  unsupported "AbsBind" "HsBindLR" (ppr e)
...
cvType (HsForAllTy a b c d) = HsForAllTy a
  <$> pure b
  <*> traverse (traverse cvHsTyVarBndr) c
  <*> traverse cvType d
