...
includes the call-site of ``error`` in ``myHead``, and of
...
details. Consider the ``myHead`` example above, the ``error`` line in
the printed stack is not particularly enlightening, so we might choose
to suppress it by freezing the ``CallStack`` that we pass to ``error``. ::
