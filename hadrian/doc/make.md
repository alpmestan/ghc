...
- Bulding a stage 1/2 GHC executable
  ...
  build _build/stage0/bin/ghc # using the actual path

  build stage2:exe:ghc-bin
  build _build/stage1/bin/ghc
  ```
...
- Building and registering a library with the stage 2 compiler
  ...
  build _build/stage1/lib/package.conf.d/text-1.2.3.0.conf # actual path
  ```
...
- Running the testsuite
  ...
  # Make
  make test
  make test TEST=plugins01
  make test TEST="plugins01 plugins02"


  # Hadrian
  build test

  build test --only=plugins01
  TEST=plugins01 build test

  build test --only="plugins01 plugins02"
  TEST="plugins01 plugins02" build test
  TEST=plugins01 build test --only=plugins02
  ```
